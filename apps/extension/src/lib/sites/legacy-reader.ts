...
} from "./xiaohongshu/state";

export type ReaderExtractionOptions = {
  archiveHtml: string;
  sourceUrl: string;
...
export function parseReadableArticle(document: Document): ReaderArticle | null {
...
export function buildGenericReaderArchive(article: ReaderArticle, currentUrl: URL) {
...
export function isXPostPage(url: URL) {
...
export function buildXPostArchive(input: {
...
export function isXiaohongshuNotePage(url: URL) {
  const hostname = url.hostname.replace(/^www\./i, "");
  return hostname === "xiaohongshu.com" && /^\/explore\/[a-z0-9]+\/?$/i.test(url.pathname);
}

export function buildXiaohongshuNoteArchive(input: {
  archivedDocument: Document;
  liveDocument?: Document | null;
  currentUrl: URL;
}) {
...
export function isSspaiPostPage(url: URL) {
  const hostname = url.hostname.replace(/^www\./i, "");
  return hostname === "sspai.com" && /^\/post\/\d+\/?$/i.test(url.pathname);
}

export function buildSspaiArticleArchive(doc: Document, currentUrl: URL) {
  const title = normalizeText(
